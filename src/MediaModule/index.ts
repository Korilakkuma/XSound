...
export class MediaModule extends SoundModule {
    ...
    for (const [key, value] of Object.entries(params)) {
      switch (key) {
        case 'mastervolume':
          ...
          break;
        case 'playbackRate':
          if (this.media && (typeof value === 'number') && (value > 0)) {
  ...
  /**
   * This method determines whether media is playing.
   * @return {boolean} If media is playing, this value is `true`. Otherwise, this value is `false`.
   */
  public playing(): boolean {
    return (this.media === null) ? false : !this.media.paused;
  }
