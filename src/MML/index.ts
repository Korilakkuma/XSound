...
import { TokenType, TokenMap, Token } from '/src/MML/Token';
import { Tree, MMLSyntaxError } from '/src/MML/Tree';
import { Sequence } from '/src/MML/Sequence';
import { Part, MMLScheduleWorkerMessageEventType, MMLScheduleWorkerMessageEventData } from '/src/MML/Part';
...
export class MML {
  ...
  public ready(params: {
    ...
    const startCallback = this.startCallback;
    const stopCallback  = this.stopCallback;
   ...
   * @return {string}
   */
  public getSyntaxTree(index: number): string {
    if ((index >= 0) && (index < this.parts.length)) {
      return this.parts[index].getSyntaxTree();
    }
    ...
    while (notes.length > 0) {
      ...
      if (/T\d+/i.test(note)) {
        ...
        while (splittedNotes.length > 0) {
          ...
          const duration = Number(splittedNote.replace(/^.+?(\d+)\.*$/, '$1'));
          ...
          if (n.includes('.')) {
            n = n.replace(/^(.+?)\d+\.+$/, `$1${1.5 * Number(n.replace(/^.+?(\d+)\.+$/, '$1'))}`);
          }

          if (n.includes('(')) {
            totalDuration += Number(n.replace(/^.+?(\d+)\.*$/i, '$1'));
          } else {
            totalDuration += d;
          }
