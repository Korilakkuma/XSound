...
export type AudioModuleParams = SoundModuleParams & {
  ...
  readonly numberOfChanels?: number
};

export type AudioModuleParam = Partial<Pick<AudioModuleParams, 'mastervolume' | 'playbackRate' | 'detune' | 'loop' | 'currentTime' | 'duration' | 'sampleRate' | 'numberOfChanels'>>;

/**
 * This subclass is for playing single audio.
 * This class enables to create audio player that has higher features than `HTMLAudioElment`.
...
export class AudioModule extends SoundModule {
  ...
  public start(startTime?: number, endTime?: number): AudioModule {
    if ((this.buffer === null) || !this.paused) {
      return this;
    }
  ...
  /**
   * This method determines whether audio is playing.
   * @return {boolean} If audio is playing, this value is `true`. Otherwise, this value is `false`.
   */
  ...
  override module(moduleName: 'vocalcanceler'): VocalCanceler;
  override module(moduleName: ModuleName | 'vocalcanceler'): Module | VocalCanceler | null {
    switch (moduleName) {
      case 'analyser':
  ...
  public params(): AudioModuleParams {
    ...
    return {
      ...
      numberOfChanels: this.buffer?.numberOfChannels ?? 0,
