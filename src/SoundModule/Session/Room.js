...
export class Room extends Connectable {
        ...
        if (this.isConnected() && this.paused) {
            ...
            this.sender.onaudioprocess = event => {
                if (this.isConnected()) {
                    ...
                    const trailer = new Float32Array(this.id.split('').map(c => c.codePointAt(0)));
                    const buffer  = new Float32Array((2 * bufferSize) + trailer.length);
                    ...
                    // Append Room ID
                    for (let i = bufferSize, len = buffer.length; i < len; i++) {
                        buffer[i] = trailer[i];
                    }
    ...
    leave() {
        if (this.websocket instanceof WebSocket) {
            const trailer = new Float32Array(this.id.split('').map(c => c.codePointAt(0)));
            const buffer  = new Float32Array(Room.BUFFER_SIZE + trailer.length);  // Empty elements + Room ID
