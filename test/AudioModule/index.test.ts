...
describe(AudioModule.name, () => {
  ...
  describe(audioModule.playing.name, () => {
    test('should return `true` if audio is playing', () => {
      expect(audioModule.playing()).toBe(false);

      // eslint-disable-next-line dot-notation
      audioModule['paused'] = false;

      expect(audioModule.playing()).toBe(true);
    });
  });
