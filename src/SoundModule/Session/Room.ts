...
export class Room implements Connectable {
  ...
  constructor(context: AudioContext, params: SessionSetupParams) {
    const { roomId, bufferSize, numberOfInputs, numberOfOutputs, analyser } = params;
    ...
    this.sender   = context.createScriptProcessor(Room.BUFFER_SIZE || bufferSize, numberOfInputs, numberOfOutputs);
    this.receiver = context.createScriptProcessor(Room.BUFFER_SIZE || bufferSize, numberOfInputs, numberOfOutputs);
  }
   ...
   */
  public join(params: SessionConnectionParams): void {
