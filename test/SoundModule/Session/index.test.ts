...
describe(Session.name, () => {
  ...
  describe(session.destroy.name, () => {
    test('should destroy session', () => {
      const originalDestroy = Room.prototype.destroy;
      const destroyMock     = jest.fn();

      Room.prototype.destroy = destroyMock;

      session.ready('test-room', params);
      session.destroy('test-room');

      expect(destroyMock).toHaveBeenCalledTimes(1);

      Room.prototype.destroy = originalDestroy;
    });
  });
