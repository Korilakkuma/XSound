...
export class AudioModule extends SoundModule {
      ...
      switch (key) {
        ...
        case 'playbackRate':
          if (typeof value === 'number') {
            if (value > 0) {
              ...
              const detune       = this.source.detune.value;
              const playbackRate = value + ((detune > 0) ? ((detune / 1200) + 1) : Math.abs(1 / ((detune / 1200) - 1)));
        ...
        case 'detune':
          if (typeof value === 'number') {
            ...
            const playbackRate = this.source.playbackRate.value + ((value > 0) ? ((value / 1200) + 1) : Math.abs(1 / ((value / 1200) - 1)));
            const startTime    = this.context.currentTime;
            const currentTime  = this.currentTime;
            const duration     = this.source.buffer?.duration ?? 0;
  ...
  public fadeIn(time?: number): number | AudioModule {
    ...
    const duration     = this.source.buffer?.duration ?? 0;
    const detune       = this.source.detune.value;
    const playbackRate = this.source.playbackRate.value + ((detune > 0) ? ((detune / 1200) + 1) : Math.abs(1 / ((detune / 1200) - 1)));
  ...
  /** @override */
  public override mix(): AudioModule {
    super.mix();
    return this;
  }

  /** @override */
  public override demix(): AudioModule {
    super.demix();
    return this;
  }
